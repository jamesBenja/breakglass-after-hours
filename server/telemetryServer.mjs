...
function summary() {
  ...
  for (const session of list) {
    for (const [npcId, count] of Object.entries(session.npcCounts)) npcRows.push({ npcId, count });
    for (const [zoneId, seconds] of Object.entries(session.zoneSeconds)) zoneRows.push({ zoneId, seconds });
  }
  const npcTotals = {};
  for (const row of npcRows) npcTotals[row.npcId] = (npcTotals[row.npcId] || 0) + row.count;
  const zoneTotals = {};
  for (const row of zoneRows) zoneTotals[row.zoneId] = (zoneTotals[row.zoneId] || 0) + row.seconds;
...
function dashboardHtml() {
  ...
  const rows=(items,unit='')=>(items||[]).map(x=>'<div class="row"><span>'+x.key+'</span><b>'+x.count+(unit||'')+'</b></div>').join('')||'<span class="muted">No data yet</span>';
...
const server = http.createServer((request, response) => {
  ...
  if (url.pathname === '/health') return json(response, 200, { ok: true, sessions: sessions.size, events: events.length });
