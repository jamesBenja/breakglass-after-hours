...
test('raw mic blobs play as audio and never as a fake synth phrase', async () => {
  ...
  try {
    ...
    assert.equal(playback.blobStems.get(vocal.id).volume, 0);
    playback.stop();
  } finally {
...
test('deleting a Spectra track removes its audio and session state', () => {
  ...
  session.attachRecording(guitar.id, { duration: 1 }, new Blob(['guitar']));
  playback.session = session;
  playback.updateMix(session);

  const removed = playback.removeStem(session, guitar.id);
  assert.equal(removed.id, guitar.id);
  assert.equal(session.stems.some((stem) => stem.id === guitar.id), false);
  assert.equal(session.recordings.has(guitar.id), false);
  assert.equal(session.recordingBlobs.has(guitar.id), false);
  assert.equal(playback.buses.has(guitar.id), false);
});
