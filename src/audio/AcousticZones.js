...
function upstairsArchive(surfaceId) {
  if (surfaceId === 'neve-suite') return environment(1, 19000, 'Neve tape playback');
  if (surfaceId === 'live-room') return environment(0.28, 3200, 'tape through studio walls');
...
function upstairsStudio(surfaceId) {
  if (surfaceId === 'mixing-suite') return environment(1, 20000, 'Spectra control room');
  if (surfaceId === 'live-room') return environment(0.74, 12000, 'live room monitor bleed');
  if (surfaceId === 'dead-room') return environment(0.46, 6800, 'dead room');
  if (surfaceId === 'neve-suite') return environment(0.28, 3600, 'studio playback through walls');
  if (surfaceId === 'circulation' || surfaceId === 'east-hall' || surfaceId === 'emergency-hall')
